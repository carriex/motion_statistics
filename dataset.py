...
class UCF101DataSet(Dataset):
    ...
    def compute_motion_label(self, v_flow_data, u_flow_data):
        ...
        v_mag, v_ang, v_frame_motion = self.compute_motion_boundary(
            v_flow_data, 'v-flow-mb')
        u_mag, u_ang, u_frame_motion = self.compute_motion_boundary(
            u_flow_data, 'u-flow-mb')
        ...
        for i in range(len(v_pattern_data)):
            v_block_id, v_angle_bin = self.compute_local_label(
                v_pattern_data[i])
            u_block_id, u_angle_bin = self.compute_local_label(
                u_pattern_data[i])
    ...
    def compute_motion_boundary(self, flow_data, title):
        ...
        for i, frame in enumerate(flow_data):
            ...
            d_x += frame_dx
            d_y += frame_dy
            frame_mag, _ = cv2.cartToPolar(
                frame_dx, frame_dy, angleInDegrees=True)
